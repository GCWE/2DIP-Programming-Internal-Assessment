...
# Tests internet connectivity (necessary for sending email)
import socket
...
def view_diagnoses():
    ...
    diagnoses_page.geometry("517x453")
    ...
    # Positions the canvas within the diagnosis page
    diagnoses_canvas.place(x = 0, y = 0)

    # The diagnosis page's title
    ...
    # The textbox in which the past diagnoses are outputted
    diagnoses_canvas.create_image(
        259.0,
        228.5,
        image=diagnoses_output_img
    )
    ...
    # The "Send Data" button - opens the "Send Diagnosis" window that allows users to email their data
    btn_diagnoses_page_send = Button(
        diagnoses_canvas,
        image=diagnoses_page_button,
        borderwidth=0,
        highlightthickness=0,
        command=email_data,
        relief="flat"
    )
    btn_diagnoses_page_send.place(
        x=171.0,
        y=373.0,
        width=176.0,
        height=71.0
    ...
    # Ensures the diagnosis page isn't resizable (as the place method is used)
...
def email_data():
    """The page through which the user's "Past Diagnoses" may be sent (via Gmail)"""

    # Identifies whether the user has internet connectivity (by checking whether Google can be opened)
    try:
    ...
    # If not, an error messagebox will pop up and the user will not be permitted to send the email
    if connection == False:
        messagebox.showerror("Error", "Wi-Fi connection is required to send diagnoses.")
        return

    # If the user has internet connectivity, the program will proceed
    # The diagnosis page is closed
    diagnoses_page.withdraw()

    # Creates the "Send Diagnoses" page
    global send_data_page
    send_data_page = Toplevel()
    send_data_page.title("Send Data")
    send_data_page.geometry("434x359")
    send_data_page.configure(bg = "#E4F0FA")

    # Builds the canvas throguh which the following widgets are built
    send_data_canvas = Canvas(
        send_data_page,
        bg = "#E4F0FA",
        height = 359,
        width = 434,
    ...
    # Positions the canvas within the "Send Diagnoses" page
    send_data_canvas.place(x = 0, y = 0)

    # The "Send Diagnoses" page's title
    send_data_canvas.create_text(
        133.0,
        40.0,
        anchor="nw",
        text="Send Data",
    ...
    # The label describing the following "Send To" input field
    send_data_canvas.create_text(
        59.0,
        117.0,
        anchor="nw",
    ...
    # The entrybox through which the user enters the email to send to
    send_data_canvas.create_image(
        297.5,
        127.73728942871094,
        image=subpage_entry_img
    )
    ...
    # The label describing that the following input field is for the user's (real) name
    send_data_canvas.create_text(
        138.0,
        165.0,
        anchor="nw",
    ...
    # The entrybox where the user enters their (real) name - this will be embedded within the email
    send_data_canvas.create_image(
        297.5,
        174.73728942871094,
        image=subpage_entry_img
    )
    global ent_email_from
    ent_email_from = Entry(
        send_data_canvas,
        bd=0,
        bg="#E3EDF7",
        fg="#000716",
    ...
    # Creates an integer variable for the following radio buttons
    global rad_var
    rad_var = IntVar()

    # The radio button which, if selected, sends all past diagnoses to the entered email
    rad_all_diagnoses = Radiobutton(
        send_data_canvas,
        bg = "#E4F0FA",
        font=Font(weight="bold", size=13 * -1),
        text="All Diagnoses",
        variable=rad_var,
        value=1
    )
    rad_all_diagnoses.place(
        x=80.0,
        y=225.0,
    )

    # The radio button which, if selected, sends the latest diagnoses to the entered email
    rad_latest = Radiobutton(
        send_data_canvas,
        bg = "#E4F0FA",
        font=Font(weight="bold", size=13 * -1),
        text="Latest",
    ...
    # The button validating whether the user's entered fields/selected results are valid
    btn_send_data = Button(
        send_data_canvas,
        image=data_page_button_img,
        borderwidth=0,
        highlightthickness=0,
        command=check_email,
        relief="flat"
    ...
    # Ensures the "Send Diagnoses" page is unresizable (as the place method is used)
    send_data_page.resizable(False, False)

def check_email():
    """
    Validates whether the "Send Diagnoses" page's inputs are valid.

    Also establishes what the email will contain (as per the user's input).    
    """

    # Obtains today's date
    current_time = datetime.now()
    timestamp = datetime.fromtimestamp(current_time.timestamp())
    str_email_timestamp = timestamp.strftime("%d %B, %Y")

    # Global declarations for the following variables
    global email_sender
    global email_password
    global email_receiver

    # The email and password created specifically for the program - the email will be sent by it
    email_sender = "gpmediagnosis@gmail.com"
    email_password = "tzfvpdnbcnbgufxv"

    # The email given by the user - this will be the email sent to
    email_receiver = ent_to_email.get()

    # Uses the "email" module to format the email
    global em
    em = EmailMessage()
    em["From"] = email_sender
    em["To"] = email_receiver

    # The regular expression used for ensuring the email is valid (i.e. in "@gmail.com" format)
    regex = r"\b[a-zA-Z0-9]+[\.{1}]?[a-zA-Z0-9]+\@gmail[\.]com\b"

    # If the email field is valid and the user wants to send all past diagnoses info
    if re.fullmatch(regex, ent_to_email.get()) and rad_var.get() == 1:
        # The title of the email
        em["Subject"] = f"GPMe: {ent_email_from.get()}'s Diagnosis Record"

        # The start of the email - explains that the following shows all the user's past diagnoses
        email_body = f"{ent_email_from.get()}'s diagnosis record, as of {str_email_timestamp}:\n\n"

        # A message for users who haven't yet gotten a diagnosis
        if keylog[ent_username.get()] == {}:
            email_body += "<< NO PAST RECORD >>"

        # Includes all past diagnoses in the message (in the same format as in the "Past Diagnoses" page)
        for timestamp in keylog[ent_username.get()]:
            email_body += f"{timestamp}:\n"
            for diagnosis in keylog[ent_username.get()][timestamp]:
                email_body +=f"- {diagnosis.title()}: {diagnosis_info[diagnosis]}\n"
            email_body += "\n" 
        
        # Ends the email with the sender's name (GPMe)
        email_body += "\n-- GPMe"

        # Writes it to the instance of the "email" module
        em.set_content(email_body)

        # Runs the function that sends the email
        send_email()       

    # If the entered email is valid and the user wants to send only the latest diagnosis
    elif re.fullmatch(regex, ent_to_email.get()) and rad_var.get() == 2:
        # The title fo the email
        em["Subject"] = f"GPMe: {ent_email_from.get()}'s Latest Diagnosis"

        # The start of the email - explains that the following shows the user's latest diagnosis
        email_body = f"{ent_email_from.get()}'s latest diagnosis, as of {str_email_timestamp}:\n\n"
        
        # A message for users who haven't yet gotten a diagnosis
        if keylog[ent_username.get()] == {}:
            email_body += "<< NO PAST RECORD >>"

        # Gets the latest diagnoses (i.e. the last item in the user's keylog dictionary)
        last_diagnosis = list(keylog[ent_username.get()]) [-1]

        # Adds the date of the latest diagnosis to the email
        email_body += f"Date of last diagnosis: {last_diagnosis}\n"

        # Adds every diagnosis from the latest date to the email
        for diagnosis in keylog[ent_username.get()][last_diagnosis]:
                email_body +=f"- {diagnosis.title()}: {diagnosis_info[diagnosis]}\n"

        # Ends the email with the sender's name
        email_body += "\n-- GPMe"
    ...
    # If the entered fields are invalid or a radio button has not been clicked, a messagebox tells the user to complete everything properly
    else:
        messagebox.showerror("Invalid", "Fill in all fields properly.")

def send_email():
    """Sends the email safely and securely to the targeted email."""

    # Uses "Secure Sockets Layer" (SSL) to send the email in privacy (especially since private information is being delivered)
    context = ssl.create_default_context()

    # Uses the Simple Mail Transfer Protocol to send the email
    with smtplib.SMTP_SSL("smtp.gmail.com", 465, context=context) as smtp:
        # Logs into the "GPMe" Gmail account
        smtp.login(email_sender, email_password)

        # Sends the email with the "GPMe" account
        smtp.sendmail(email_sender, email_receiver, em.as_string())

    # Closes the "Send Diagnoses" page
    send_data_page.withdraw()

    # Outputs a messagebox telling the user the email was successfully delivered
    messagebox.showinfo("Email Delivered", "Diagnoses successfully sent!")
...
diagnosis_info = {
    ...
    "heart pain": "Call a doctor or 111 if intensely painful or worrying",
    "toothache": "Should self-disperse - call dentist after two days. Eat soft food.",
    "shoulder pain": "Regularly exercise shoulder gently - call doctor if it doesn't subside within two weeks."
}
...
diagnoses_output_img = PhotoImage(
    file="images/diagnoses_output.png")

# The "Send Data" button on the "View Diagnoses" page
diagnoses_page_button = PhotoImage(
    file="images/diagnoses_send.png")

# The "Send Data" button on the "Send Data" page
data_page_button_img = PhotoImage(
    file="images/data_page_button.png")
